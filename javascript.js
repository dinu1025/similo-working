...
function locateElementByXPath(xPath) {
  ...
  console.log(nodesSnapshot.snapshotLength);
...
function transfAddText(xPath, element) {
  ...
  if (
    ...
    newXPath.addPredicateToHead(
      "[contains(text(),'" + cleanText(ancestor.textContent) + "')]"
    );
    output.push(newXPath);
  }
  return output;
  ...
  if (!xPath.headHasAnyPredicates()) {
    ...
    for (const attribute of ancestor.attributes) {
      if (
        ...
        !attributePriorizationList.includes(attribute.name)
      ) {
        const newXPath = new XPath(xPath.getValue());
        newXPath.addPredicateToHead(
          "[@" + attribute.name + "='" + attribute.value + "']"
...
function transfAddAttributeSet(xPath, element) {
  ...
  if (!xPath.headHasAnyPredicates()) {
    ...
    attributes = attributes.filter(
      (attribute) => !attributeBlackList.includes(attribute.name)
    );
    ...
    for (const attributeSet of attributePowerSet) {
      let predicate =
        "[@" + attributeSet[0].name + "='" + attributeSet[0].value + "'";
      for (let i = 1; i < attributeSet.length; i++) {
        predicate +=
          " and @" + attributeSet[i].name + "='" + attributeSet[i].value + "'";
      }
      predicate += "]";
      const newXPath = new XPath(xPath.getValue());
...
function getMonotoXPath(element) {
  ...
  while (true) {
    if (
      (n.children.length == 0 ||
        (n.children.length == 1 && n.children[0].nodeType == 3)) &&
      n.textContent.length > 0
    ) {
      let xpath =
        "//" + n.tagName + "[text()='" + escapeQuotes(n.textContent) + "']" + x;
...
function xpathImg(e) {
  if (e.nodeName == "IMG") {
    if (e.alt != "") {
      return preciseXPath(
        "//" + xpathHtmlElement("img") + "[@alt=" + attributeValue(e.alt) + "]",
        e
      );
    } else if (e.title != "") {
      return preciseXPath(
